...
void NeoPixel_SendBit(uint8_t bit)
{
    if(bit)
    {
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ldr_value = Read_ADC_Channel(ADC_CHANNEL_0);
	  pot_value = Read_ADC_Channel(ADC_CHANNEL_1);

	  distance = Read_Ultrasonic();

	  temp = 30 + (pot_value / 500);
	  hum = 60 + (ldr_value / 200);
	  ...
	  // Patient near edge → alarm
	  if(distance < 15)
	  {
		  ssd1306_WriteText("ALERT ON");
	      for(int i=0;i<100;i++)
	      {
	      ...
	      NeoPixel_SetColor(0,0,255);
	  }
	  else if(temp > 32)
	  {
		  ssd1306_WriteText("TEMP HIGH");
	      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_RESET);
	      HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_RESET);
	      NeoPixel_SetColor(255,255,0);
	  }
	  else
	  {
		  ssd1306_WriteText("FAN ON");
	      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_RESET);
	      HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_RESET);
	      NeoPixel_SetColor(0,255,0);
	  }
